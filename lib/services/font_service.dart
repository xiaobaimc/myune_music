...
class FontService {
  ...
  Future<List<FontMeta>> scanFonts() async {
    ...
    if (cached != null) {
      ...
      final results = await Future.wait(cached.entries.map((entry) async {
        ...
        if (filePath.toLowerCase().endsWith('.ttc')) {
          ...
          if (familyNames.isEmpty) {
            ...
            for (int idx = 0; idx < familyNames.length; idx++) {
              ...
              final uniqueKey = familyNames.length > 1 ? '${fileName}_$idx' : fileName;
    ...
    if (meta.filePath.endsWith('.ttc')) {
      try {
        ...
        if (fileSize > maxFileSize) {
          debugPrint('TTC font file too large: ${meta.fileName} (${fileSize ~/ (1024 * 1024)}MB)');
          meta.isLoaded = false;
          return;
        }
          ...
          if (entity is File && entity.path.toLowerCase().endsWith('.ttc')) {
            ...
            try {
              await for (final subEntity in entity.list()) {
                if (subEntity is File && subEntity.path.toLowerCase().endsWith('.ttc')) {
  ...
  List<String> _getFontDirectories() {
    ...
    if (Platform.isWindows) {
      ...
      return [
        ...
        if (userProfile != null) '$userProfile/AppData/Local/Microsoft/Windows/Fonts/',
      ];
    }
    // macOS平台字体目录
    if (Platform.isMacOS) {
  ...
  Future<Uint8List> _readBytes(
    ...
    if (length > maxLength) {
      throw Exception('Requested length $length exceeds maximum allowed $maxLength');
    }
    await raf.setPosition(position);
    final result = await raf.read(length);
    if (result.length < length) {
      throw Exception('Unexpected end of file: expected $length bytes, got ${result.length}');
    }
    return result;
  }
